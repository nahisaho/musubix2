...
```
Phase 1: 交互レビュー
   Round 1: Model A (Claude Opus 4.6) がドキュメントをレビュー → issues を検出
   Round 2: Model B (GPT-5.4) が issues を受け取りレビュー → 追加/修正
   Round 3: Model A が Model B の指摘を踏まえ再レビュー
   Round 4: Model B が再レビュー
   ...エラー 0 になるか、最大 5 ラウンドまで繰り返し

Phase 2: 最終合意チェック（finalConsensusCheck）
   両モデルが並列にレビュー（Promise.all）
   → 両方 PASS の場合のみ承認

Phase 3: パイプラインレビュー
   requirements → design → plan の順に実行
   → 1つでも不合格なら後続をブロック
   → 全て承認済みで実装フェーズへ遷移可能
```
...
```
MUSUBIX2 のニューロシンボリック層（Spec Kit / Kiro にはない）

ニューラル側:
  ├── neural-search:    TF-IDF 類似度検索（類似要件・コード重複検出）
  ├── wake-sleep:       N-gram パターン学習（コードパターン自動発見）
  ├── library-learner:  E-graph 等価クラス（リファクタリング候補発見）
  └── deep-research:    証拠チェーン付きリサーチ

シンボリック側:
  ├── formal-verify:    EARS → SMT-LIB2 → Z3 自動検証
  ├── lean:             Lean 4 定理証明
...
```
カテゴリ 1: AI コーディングアシスタント（コードを書く "手"）
    ├── GitHub Copilot  — インライン補完、最も広く普及
    ├── Cursor           — VS Code フォーク、エージェント型 IDE
    ├── Claude Code      — ターミナル型、100万トークンコンテキスト
    └── Windsurf         — VS Code フォーク、Cascade エージェント

カテゴリ 2: 自律型 AI エンジニア（計画から実装まで自律実行）
    └── Devin            — 完全自律、サンドボックス環境

カテゴリ 3: 仕様駆動開発システム（仕様を管理し、正しさを保証する "頭脳"）
    ├── GitHub Spec Kit  — テンプレート + CLI
    ├── AWS Kiro         — 専用 IDE
...
│  MUSUBIX2（"頭脳"）                               │
│                                                   │
│  ・EARS 要件管理     ・形式検証（Z3 / Lean 4）     │
│  ・SOLID 設計生成    ・マルチモデル交互レビュー     │
│  ・タスク分解        ・100% トレーサビリティ        │
│  ・ニューラル検索    ・知識グラフ                   │
│  ・パターン学習      ・Git 履歴分析                 │
│  ・9条憲法チェック   ・品質ゲート                   │
