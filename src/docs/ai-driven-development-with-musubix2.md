---
title: MUSUBIX2 によるAI駆動型アプリケーション開発
tags:
  - MUSUBIX2
  - SDD
  - ニューロシンボリック
...
```mermaid
flowchart TD
    P1["Phase 1: Requirements（要件定義）"]
    P1a["EARS 形式で要件を記述"]
    P1b["⏸️ ユーザーレビュー → 承認"]
    P2["Phase 2: Design（設計）"]
    P2a["SOLID 準拠の設計書を生成"]
    P2b["⏸️ ユーザーレビュー → 承認"]
    P3["Phase 3: Task Breakdown（タスク分解）"]
    P3a["設計から実装タスクを分解"]
    P3b["⏸️ ユーザーレビュー → 承認"]
    P4["Phase 4: Implementation（実装）"]
    P4a["Red → Green → Blue サイクル"]
    P5["Phase 5: Complete（完了）"]
    P5a["全品質ゲート通過"]
...
flowchart TD
    ...
    Input --> Quality{"品質関連?"}
    Input --> Analysis{"分析関連?"}
    Input --> Knowledge{"知識関連?"}
    Input --> Learn{"学習関連?"}

    Spec -->|要件| RA["requirements-analyst"]
    Spec -->|情報不足| RA2["requirements-analyst（1問1答）"]
    Spec -->|設計| DG["design-generator"]

    Impl -->|コード生成| CG["code-generator"]
    Impl -->|テスト| TE["test-engineer"]
    Impl -->|合成| SY["synthesis"]

    Quality -->|トレーサビリティ| TA["traceability-auditor"]
    Quality -->|ポリシー| CE["constitution-enforcer"]
    Quality -->|レビュー| RO["review-orchestrator"]
    Quality -->|形式検証| FV["formal-verify / lean"]

    Analysis -->|AST| CGraph["codegraph"]
    Analysis -->|データフロー| DFG["dfg"]
    Analysis -->|セキュリティ| SEC["security"]

    Knowledge -->|知識グラフ| KG["knowledge"]
    Knowledge -->|Git 履歴| GK["git-knowledge"]
    Knowledge -->|ADR| DEC["decisions"]

    Learn -->|ニューラル検索| NS["neural-search"]
    Learn -->|パターン学習| LL["library-learner"]
    Learn -->|Wake-Sleep| WS["wake-sleep"]
```
...
flowchart TD
    ...
    User --> ReqExist{"Phase 1 の要件定義書が存在する？"}
    ReqExist -->|NO| GoReq["「まず要件を定義します」"]
    ReqExist -->|YES| ReqApproved{"承認済み？"}
    ReqApproved -->|NO| GoReview1["「要件のレビューが必要です」"]
    ReqApproved -->|YES| DesApproved{"Phase 2 の設計書が承認済み？"}
    DesApproved -->|NO| GoDes["「設計を行います」"]
    DesApproved -->|YES| TaskApproved{"Phase 3 のタスク分解が承認済み？"}
    TaskApproved -->|NO| GoTask["「タスクを分解します」"]
    TaskApproved -->|YES| OK["✅ 実装開始許可"]
```
...
flowchart TD
    ...
    Editor -->|stdio| Stdio["StdioTransport"]
    Editor -->|HTTP| SSE["SSETransport"]
...
これは他のどの AI ツールにもない特徴です。Cursor や Claude Code はコード補完・生成に優れますが、**ドキュメントの品質を AI がレビューする仕組み**を持っていません。MUSUBIX2 は仕様書そのものを AI に相互チェックさせます。
...
```mermaid
sequenceDiagram
    participant A as Model A（Claude Opus 4.6）
    participant B as Model B（GPT-5.4）
    participant O as Orchestrator

    Note over O: Phase 1: 交互レビュー
    O->>A: Round 1: ドキュメントレビュー依頼
    A-->>O: issues 検出
    O->>B: Round 2: issues を渡してレビュー依頼
    B-->>O: 追加/修正 issues
    O->>A: Round 3: 再レビュー
    A-->>O: issues（0件 or 追加）
    O->>B: Round 4: 再レビュー
    B-->>O: issues
    Note over O: エラー 0 or 最大 5 ラウンドまで

    Note over O: Phase 2: 最終合意チェック
    par 並列レビュー（Promise.all）
        O->>A: 最終レビュー
        O->>B: 最終レビュー
    end
    A-->>O: PASS / FAIL
    B-->>O: PASS / FAIL
    Note over O: 両方 PASS → 承認

    Note over O: Phase 3: パイプラインレビュー
    O->>O: requirements → design → plan 順に実行
    Note over O: 1つでも不合格 → 後続ブロック
    Note over O: 全承認 → 実装フェーズへ遷移可能
```
...
Spec Kit と Kiro は「仕様 → タスク」のリンクは管理しますが、**コードやテストまで含めた End-to-End のトレーサビリティ**は提供していません。MUSUBIX2 は `REQ-XXX-NNN ↔ DES-XXX-NNN ↔ src/ ↔ tests/` の全方向リンクを管理し、ギャップ（追跡漏れ）を自動検出します。
...
Spec Kit と Kiro は「仕様ファースト」の文化を広めるための優れたツールですが、あくまで**仕様管理**に特化しています。MUSUBIX2 は仕様管理に加えて、**ニューロシンボリック AI による自動検証・学習・推論**という独自の層を持ち、仕様の正しさを**数学的に保証**できるシステムです。


# 11. AI コーディングツールとの比較 — Cursor / Claude Code / Copilot / Windsurf / Devin

SDD ツール（Spec Kit / Kiro）との比較に加えて、AI コーディングツール全般との位置づけも重要です。MUSUBIX2 はこれらのツールと**競合するのではなく、補完する**関係にあります。
...
1. **EARS → SMT-LIB2 → Z3 自動検証**: 要件の論理的一貫性を数学的に証明
2. **Lean 4 定理証明**: 安全性要件を形式的に証明
3. **REQ↔DES↔Code↔Test 100% トレーサビリティ**: 変更が仕様のどこに影響するか即座に特定
4. **マルチモデルドキュメントレビュー**: 要件定義書・設計書・計画書を Claude Opus 4.6 × GPT-5.4 が交互レビュー（最大5ラウンド + 最終合意チェック）
5. **Git log/blame → 知識グラフ**: プロジェクトの暗黙知を構造化
6. **9条憲法**: AI が守るべき不変ルールの自動検証
7. **1問1答インタビュー**: 情報不足時の構造化ヒアリング

**結論**: AI コーディングツールは「コードを速く書く」ことに優れています。しかし**正しいコードを書く**ためには、仕様の厳密な管理と検証が不可欠です。MUSUBIX2 はその「正しさの保証」を担うシステムであり、AI コーディングツールと組み合わせることで最大の効果を発揮します。
...
- **GitHub Spec Kit**: [github.com/github/spec-kit](https://github.com/github/spec-kit) — [GitHub Blog](https://github.blog/ai-and-ml/generative-ai/spec-driven-development-with-ai-get-started-with-a-new-open-source-toolkit/)
- **AWS Kiro**: [kiro.dev](https://kiro.dev/blog/introducing-kiro/) — [InfoQ 解説](https://www.infoq.com/news/2025/08/aws-kiro-spec-driven-agent/)
- **Cursor**: [cursor.com](https://cursor.com)
- **Claude Code**: [anthropic.com](https://docs.anthropic.com/en/docs/agents-and-tools/claude-code/overview)
